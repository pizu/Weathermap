...
// nodename is a vestigal parameter, from the days when nodes were just big labels
	...
	if (empty($this->datestamp)) {
            $ts = $this->map_time ?: time();
            $this->datestamp = wm_format_date($this->stamptext, $ts);
        }
...
// we skip any gradient scales
    ...
    function DrawMap(
        ...
        $this->map_time = $maptime;
        $this->datestamp = wm_format_date($this->stamptext, $this->map_time);
...
//          TODO: check that the antialias turn off doesn't cause any problems
